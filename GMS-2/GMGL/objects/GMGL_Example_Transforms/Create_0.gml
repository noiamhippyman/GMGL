...
*/
gmgl_create_window_centered(800,600,"Example - Transforms");
...
*/
var vertShader = gl_create_shader(GL_VERTEX_SHADER,shader_example_transforms_vs());
var fragShader = gl_create_shader(GL_FRAGMENT_SHADER,shader_example_transforms_fs());
...
/*
	In order to apply transforms we need to use matrices
	Since we can't send arrays directly to non-gml extensions
	We have to write the array to a buffer and send that
	with buffer_get_address
	
	Since we're dealing with 4x4 matrices for transforms
	the buffer needs to hold 16 float values. Since they're 
	32 bit floats the alignment needs to be 4 bytes. That way
	the buffer_write functions will work properly and you only 
	have to seek a single time. 
	
	You can see an example of this in the step event
*/
transformMatrixBuffer = buffer_create(16*buffer_sizeof(buffer_f32),buffer_fixed,4);